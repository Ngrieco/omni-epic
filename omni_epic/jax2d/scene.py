...
def add_circle_to_scene(
	...
	new_sim_state = sim_state.replace(
		circle=sim_state.circle.replace(
			...
			angular_velocity=sim_state.circle.angular_velocity.at[circle_index].set(
				angular_velocity
			),
			...
			color=sim_state.circle.color.at[circle_index].set(color),
		)
	)

	return (
		jax.tree_util.tree_map(
			lambda x, y: jax.lax.select(can_add_circle, x, y), new_sim_state, sim_state
		),
		(
			circle_index,
...
def add_polygon_to_scene(
	...
	inverse_mass, delta_centre_of_mass = calc_inverse_mass_polygon(
		vertices, n_vertices, static_sim_params, density
	)
	...
	new_sim_state = sim_state.replace(
		polygon=sim_state.polygon.replace(
			...
			angular_velocity=sim_state.polygon.angular_velocity.at[polygon_index].set(
				angular_velocity
			),
			...
			inverse_inertia=sim_state.polygon.inverse_inertia.at[polygon_index].set(
				inverse_inertia
			),
			...
			color=sim_state.polygon.color.at[polygon_index].set(color),
		)
	)

	return (
		jax.tree_util.tree_map(
			lambda x, y: jax.lax.select(can_add_polygon, x, y), new_sim_state, sim_state
		),
