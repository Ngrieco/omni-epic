...
def init_archive(archive_from_ckpt):
	...
	if len(archive_from_ckpt) > 0:
		...
		with open(archive_from_ckpt) as f:
			content = f.read()
			json_str = re.split("(?<=})\n(?={)", content)[-1]
...
def main(config: DictConfig):
	...
	archive_codepaths, archive_failedgens, archive_failedint, archive_failedtrain = init_archive(
		config.archive_from_ckpt
	)
	init_archive_size = len(task_descs_init)
	prev_num_iterations = (
		len(archive_codepaths)
		+ len(archive_failedgens)
		+ len(archive_failedint)
		+ len(archive_failedtrain)
	)
	...
	iterations_spent_on_init_tasks = (
		0  # iterations spent on generating tasks from task_descs_init this run
	)
	taskgen_choose_probs = np.ones(
		len(archive_codepaths)
	)  # probability of choosing a task from the archive
	stop_iteration = (
		False  # stop iterations, only used when config.iterate_until_success_gen is True
	)
	...
	for iteration in range(config.iterations):
		...
		if iteration < len(task_descs_init):
			...
			if not config.use_archive:
				...
				config_task_iterator.num_examples > 0
				and len(archive_codepaths) > config_task_iterator.num_examples
			):
			...
			if config_success_detector.use_vision:
				...
				taskgen_completion = fm_task_iterator.reflect_task_with_vision(
					...
					taskgen_example_paths,
					success_reasoning,
					input_image,  # should have been initialized in the previous iteration
			...
			if not config.use_archive:
				...
				num_examples = min(
					num_examples,
					max(
						math.ceil((iteration + 1 - init_archive_size) / init_archive_size),
						1,
					),
				)
				...
				probs = (
					taskgen_choose_probs
					if np.any(taskgen_choose_probs)
					else np.ones(len(archive_codepaths))
				)
				probs /= np.sum(probs)
			...
			taskgen_completion = fm_env_generator.query_env_code(
				...
				task_desc,
				add_examples=add_examples,
				env_paths_other=taskgen_example_paths + taskgen_add_example_paths,
			)
		...
		if gen_num >= 0:
			...
			if (
				config.use_archive
				and config.enable_moi
				and len(archive_codepaths) > iterations_spent_on_init_tasks
			):
			...
			if is_interesting:
				if config.train_agent:
					...
					if (
						config.train_from_ckpt
						and len(archive_codepaths) > 0
					...
					if config.enable_sd and config_success_detector.use_vision:
						...
						video_files = [
							...
							if f.endswith(".mp4") and f.startswith("render")
						]
						...
						_, task_success, success_reasoning = (
							fm_success_detector.query_success_with_vision(
								robot,
								robot_desc,
								task_desc,
								task_envpath,
